...
def send_email(to_email: str, subject: str, body_text: str, body_html: str | None):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="SMTP server refused the email data.")
    except smtplib.SMTPServerDisconnected:
        raise HTTPException(status_code=500, detail="SMTP server disconnected unexpectedly.")
    except smtplib.SMTPException:
        raise HTTPException(status_code=500, detail="An SMTP error occurred.")
    except Exception:
        raise HTTPException(status_code=500, detail="An unknown error occurred while sending email.")
