...
async def add_user_product_like(
    ...
    result = await session.execute(
        select(Product).where(
            Product.id == user_product_like_in.product_id
        )
    )
    ...
    await session.execute(
        user_product_likes.insert().values(
            user_id=user.id,
            product_id=user_product_like_in.product_id
        )
    )
...
async def get_user_product_likes(
    ...
    result = await session.execute(
        user_product_likes.select().where(
            user_product_likes.c.user_id == user.id
        )
    )
    user_likes = result.all()
    return [UserProductLike(product_id=user_like.product_id) for user_like in user_likes]
...
async def add_bulk_user_product_like(
    ...
    product_ids_to_add = [
        pid for pid in user_product_likes_in.product_ids
