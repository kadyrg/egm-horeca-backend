...
def upgrade() -> None:
    ...
    op.add_column(
        "product_translation", sa.Column("product_id", sa.Integer(), nullable=False)
    )
    op.drop_constraint(
        op.f("product_translation_language_id_fkey"),
        "product_translation",
        type_="foreignkey",
    )
    op.create_foreign_key(
        None, "product_translation", "language_codes", ["language_code_id"], ["id"]
    )
    op.create_foreign_key(
        None, "product_translation", "products", ["product_id"], ["id"]
    )
...
def downgrade() -> None:
    ...
    op.create_unique_constraint(
        op.f("categories_slug_ro_key"),
        "categories",
        ["slug_ro"],
        postgresql_nulls_not_distinct=False,
    )
    op.create_unique_constraint(
        op.f("categories_slug_en_key"),
        "categories",
        ["slug_en"],
        postgresql_nulls_not_distinct=False,
    )
    op.create_unique_constraint(
        op.f("categories_name_ro_key"),
        "categories",
        ["name_ro"],
        postgresql_nulls_not_distinct=False,
    )
    op.create_unique_constraint(
        op.f("categories_name_en_key"),
        "categories",
        ["name_en"],
        postgresql_nulls_not_distinct=False,
    )
